...
import { employee, AttendanceService } from 'src/app/managers/attendance-tab/attendance.service';
import { VerifyFormDepartureComponent } from 'src/app/employees/attendance/verify-form-departure/verify-form-departure.component';
import { VerifyFormArrivalComponent } from 'src/app/employees/attendance/verify-form-arrival/verify-form-arrival.component';
...
export class AttendancePage implements OnInit {
  ...
  async verifymodal()
  {
    const modal = await this.modal.create({
      component:  VerifyFormArrivalComponent
    });
   await modal.present();
  }
  ...
  {
    const time = Date.now()
    let date = this.pipe.transform(time, 'mediumTime');
    this.attendance.getCodeDateOrder(date).subscribe((data: any) => {
      this.form = data;
      this.qrdata = data[0].qrcode;
